...
exports.getUpdatedDashboardHtmlString = exports.getCertificateHtmlString = void 0;
...
function getCertificateHtmlString(name) {
    return [
        `<html>`,
        `\t<body>`,
...
exports.getCertificateHtmlString = getCertificateHtmlString;
function getDaysLevelTooltipText(level) {
    switch (level) {
        case 0:
...
function getHoursLevelTooltipText(level) {
    switch (level) {
        case 0:
...
function getStreaksLevelTooltipText(level) {
    switch (level) {
        case 0:
...
function getLinesAddedLevelTooltipText(level) {
    switch (level) {
        case 0:
...
function generateShareUrl(days, hours, streaks, linesAdded, avgHours) {
    const hashtagURI = "%23";
    const shareText = [
        `\n\nDays coded: ${days}`,
...
function getDatagramHtmlComponents() {
    // Datagram
    const codeTimeHours = LogsUtil_1.getAllCodetimeHours();
    let max = 0;
    let mid = 0;
    const min = 0;
    let barsHtml = "";
    ...
    if (codeTimeHours.length > 0) {
        ...
        let datesFromLogs = LogsUtil_1.getLogDateRange();
...
function getLogsHtml() {
    // Logs
    const logs = LogsUtil_1.getLastSevenLoggedDays();
    let logsHtml = "";
    const d = new Date();
    if (logs.length === 0) {
...
function getMilestonesHtml(recent_milestones) {
    // Milestones
    let milestoneHtml = "";
    if (recent_milestones.length > 0) {
        ...
        for (let i = 0; i < recent_milestones.length; i++) {
            ...
            const milestone = MilestonesUtil_1.getMilestoneById(milestoneId);
            if (milestone) {
                milestoneHtml += [
...
function getUpdatedDashboardHtmlString() {
    ...
    const { daysLevel, daysProgressPercentage } = SummaryUtil_1.getDaysLevel(days);
    const { hoursLevel, hoursProgressPercentage } = SummaryUtil_1.getHoursLevel(hours);
    const { streaksLevel, streaksProgressPercentage } = SummaryUtil_1.getLongStreakLevel(streaks);
    const { linesAddedLevel, linesAddedProgressPercentage } = SummaryUtil_1.getLinesAddedLevel(linesAdded);
    ...
    const avgHoursLevelTooltip = avgHours >= 1 ? "Great job! You're on track to coding one hour every day." : "Try to code an hour each day.";
    const twitterShareUrl = generateShareUrl(days, hours, streaks, linesAdded, avgHours);
    const { cardTextColor, cardBackgroundColor, datagramXMinColor, datagramBackground, cardToolTipColor } = getStyleColorsBasedOnMode();
...
exports.getUpdatedDashboardHtmlString = getUpdatedDashboardHtmlString;
