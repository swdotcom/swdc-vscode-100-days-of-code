...
export function achievedMilestonesJson(id: number){
    ...
    var jsonData = {"milestones": milestones};
...
export function milestoneShareUrlGenerator(id: number, title: string, description: string): string{
    ...
    const endURI = "%0aWoohoo%21%20I%20earned%20this%20milestone%20while%20completing%20the%20%23100DaysOfCode%20Challenge%20with%20@software_hq%27s%20100%20Days%20of%20Code%20VS%20Code%20Plugin.&hashtags=100DaysOfCode%2CSoftware%2CDeveloper%2CAchiever";
...
export function getUpdatedMilestonesHtmlString(){
    ...
    ​
            ...
            const milestoneCardHtml: string = [`\t\t<div class="milestoneCard ${grayedCard}">`,
            `${levelHtml}`,
            `${shareHtml}`,
...
export function updateMilestonesHtml(){
    ...
    fs.writeFile(filepath, getUpdatedMilestonesHtmlString() ,(err) => {
        if (err) {
            console.error(err);
        }
    });
